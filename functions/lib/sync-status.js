...
export async function buildSyncStatusReport(db, { lastChunk } = {}) {
  ...
  const sync_driver = await getSyncDriver(db);
        ...
        pausedManual
          ...
          : runState === 'running'
            ? sync_driver === 'github'
              ...
              : cron.enabled
                ? 'Sync bulanan: burst ~2×10 hal × 20 sekolah/hal ≈ 400/menit bila fp_skip tinggi. API data pusat maks 20 baris/request.'
...
<body class="bg-slate-50 p-6 font-sans">
  <div class="max-w-lg mx-auto bg-white rounded-xl border border-amber-200 p-6 shadow-sm">
    ...
    <ol class="text-sm text-slate-600 list-decimal list-inside space-y-2 mb-4">
      ...
      <li>Jalankan konfigurasi D1: <code class="block mt-1 bg-slate-900 text-emerald-300 text-xs p-3 rounded">npx wrangler d1 create api-sekolah-db</code></li>
      <li>Update wrangler.toml dengan ID database</li>
      <li>Deploy ulang: <code class="bg-slate-100 px-1 rounded">npm run deploy</code></li>
      <li>Jalankan migrasi: <code class="bg-slate-100 px-1 rounded">npx wrangler d1 migrations apply api-sekolah-db --remote</code></li>
    </ol>
